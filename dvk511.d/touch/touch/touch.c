...
{
	u8 tmp1,tmp2; u16  tmp; dis_lcd(); en_touch();
	delayMicroseconds(2);
...
void lcd_draw_dot(unsigned int color_front, unsigned char x, unsigned int y)
{
...
void lcd_draw_bigdot(unsigned int   color_front, unsigned int    x, unsigned int    y )
{
...
void main(void)
{
	...
	LCD_test();
	DisplayString("John",6,9,0);
	DisplayString("Wiggins",6,10,1);
	while(1)
	{
//	display_touch_debug();		
	read_res=Read_Continue();
	//lcd_draw_bigdot(0xffe0,(touch_dot.x-20),(touch_dot.y-2));
	lcd_draw_bigdot(CLR_WHITE,(touch_dot.x-20),(touch_dot.y-2));
	}
}
