...
static unsigned int o_pcfpin = 254; /* wiringPi numbering scheme */
...
int activity(FILE *vmstat)
{
        ...
        if (!found_pgpgin || !found_pgpgout)
	{
        ...
        /* Anything changed? */
        result = (prev_pgpgin  != pgpgin) || (prev_pgpgout != pgpgout);
        prev_pgpgin = pgpgin;
...
void led(int on)
{
	...
	{
		// turn on
		if ((wiringPiI2CWrite(fd,o_pcfpin)) < 0)
		{
		    fprintf (stderr, "write badly %s\n", strerror (errno));
		    return 1;
		}
        	//digitalWrite (o_pcfpin, HIGH);
	}
	else
	{
		// turn off
...
error_t parse_options(int key, char *arg, struct argp_state *state)
{
        switch (key)
	{
        ...
        case 'r':
                ...
                break;
        case 'p':
                o_pcfpin = strtol(arg, NULL, 10);
                if ((o_pcfpin < 0) || (o_pcfpin > 20))
                        argp_failure(state, EXIT_FAILURE, 0,
                                "pin number must be between 0 and 20");
                break;
        }
        return 0;
}
...
int main(int argc, char **argv)
{
        struct argp_option options[] = {
                ...
                { "pin",     'p',   "VALUE", 0, "PCF pin where LED is connected (default: 254, p0)" },
