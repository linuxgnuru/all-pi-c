/* LCD SPI 4-bit Version 2 (hard-wired 74HC595-based) DEMOnstration code
 * $Id: lcd_spi4bV2_demo.c,v 1.7 2018/03/31 21:32:45 pi Exp $
 */
...
static char RCSFile[]     = "$RCSfile: lcd_spi4bV2_demo.c,v $";
static char RCSId[]       = "$Id: lcd_spi4bV2_demo.c,v 1.7 2018/03/31 21:32:45 pi Exp $";
...
#ifdef HAVE_WIRINGPI_H
void C2S_LcdS4V2Strobe_Test(char qx)
{
  unsigned int lS4data;
  lS4data = current_spi_4bitV2.lcd_S4data;
  current_spi_4bitV2.lcd_S4data = lS4data | SPI4BIT_LCD_E_HIGH;
  C2S_sechob2spidev(current_spi_4bitV2.lcd_data.spidev,
		    current_spi_4bitV2.lcd_S4data, 
		    qx);
  sleep(1);
  current_spi_4bitV2.lcd_S4data = lS4data & SPI4BIT_LCD_E_LOW;
  C2S_sechob2spidev(current_spi_4bitV2.lcd_data.spidev,
		    current_spi_4bitV2.lcd_S4data, 
		    qx);
  current_spi_4bitV2.lcd_S4data = lS4data;
  sleep(1);
}

void C2S_LcdS4V2WriteRegister_Test(unsigned int regh,
				   unsigned int regl,
				   char qx)
{
  current_spi_4bitV2.lcd_S4data = SPI4BIT_LCD_RMASK + (regh * 0x10);
  C2S_LcdS4V2Strobe_Test(qx);
  current_spi_4bitV2.lcd_S4data = SPI4BIT_LCD_RMASK + (regl * 0x10);
  C2S_LcdS4V2Strobe_Test(qx);
}

void C2S_LcdS4V2WriteData_Test(unsigned int data,
			       char qx)
{
  unsigned int datah,datal;
  datah = data & 0xf0;
  datal = data & 0x0f;
  current_spi_4bitV2.lcd_S4data = SPI4BIT_LCD_DMASK + (datah);
  C2S_LcdS4V2Strobe_Test(qx);
  current_spi_4bitV2.lcd_S4data = SPI4BIT_LCD_DMASK + (datal * 0x10);
  C2S_LcdS4V2Strobe_Test(qx);
}

#ifdef HAVE_WIRINGPISPI_H
void WP_LcdS4V2Strobe(lcd_spi_4bitV2 *any_spi_4bitV2,
		      char qx)
{
  unsigned int lS4data,lwait;
  char uorm;
  uorm = TRUE;
  lS4data = (*any_spi_4bitV2).lcd_S4data;
  if ((*any_spi_4bitV2).S4bit.lcd_wait > (*any_spi_4bitV2).S4bit.lcd_elmin)
    lwait = (*any_spi_4bitV2).S4bit.lcd_wait;
  else lwait = (*any_spi_4bitV2).S4bit.lcd_elmin;
  if (lwait > 9999)
    {
      uorm = FALSE;
      lwait = lwait/1000;
    }
  (*any_spi_4bitV2).lcd_S4data = lS4data | SPI4BIT_LCD_E_HIGH;
  WP_write_byte(*(*any_spi_4bitV2).lcd_data.spifd,
		(*any_spi_4bitV2).lcd_S4data, 
		qx);
  delayMicroseconds((*any_spi_4bitV2).S4bit.lcd_ehmin);
  (*any_spi_4bitV2).lcd_S4data = lS4data & SPI4BIT_LCD_E_LOW;
  WP_write_byte(*(*any_spi_4bitV2).lcd_data.spifd,
		(*any_spi_4bitV2).lcd_S4data, 
		qx);
  (*any_spi_4bitV2).lcd_S4data = lS4data;
  /* Wait for lcd to complete operation */
  if (uorm) delayMicroseconds(lwait);
  else delay(lwait);
}

void WP_LcdS4V2F4TStrobe(lcd_spi_4bitV2 *any_spi_4bitV2,
			 int instance,
			 char qx)
{
  unsigned int lS4data,lwait;
  ...
  (*any_spi_4bitV2).lcd_S4data = SPI4BIT_LCD_RMASK + (LCD_FUNC_SET_4BIT_INIT * 0x10);
  if (instance > 2) (*any_spi_4bitV2).lcd_S4data = SPI4BIT_LCD_RMASK +
  ...
  (*any_spi_4bitV2).lcd_S4data = (*any_spi_4bitV2).lcd_S4data | SPI4BIT_LCD_E_HIGH;
  WP_write_byte(*(*any_spi_4bitV2).lcd_data.spifd,
		(*any_spi_4bitV2).lcd_S4data, 
		qx);
  delayMicroseconds((*any_spi_4bitV2).S4bit.lcd_ehmin);
  (*any_spi_4bitV2).lcd_S4data = (*any_spi_4bitV2).lcd_S4data & SPI4BIT_LCD_E_LOW;
  WP_write_byte(*(*any_spi_4bitV2).lcd_data.spifd,
		(*any_spi_4bitV2).lcd_S4data, 
		qx);
  (*any_spi_4bitV2).lcd_S4data = lS4data;
...
void WP_LcdS4V2WriteRegister(lcd_spi_4bitV2 *any_spi_4bitV2,
			     unsigned int regh,
			     unsigned int regl,
			     char qx)
{
  unsigned int old_lcd_wait;
  (*any_spi_4bitV2).lcd_S4data = SPI4BIT_LCD_RMASK + (regh * 0x10);
  WP_LcdS4V2Strobe(any_spi_4bitV2, qx);
  (*any_spi_4bitV2).lcd_S4data = SPI4BIT_LCD_RMASK + (regl * 0x10);
  if (regl == LCD_RETURN_HOME_L)
    {
      old_lcd_wait = (*any_spi_4bitV2).S4bit.lcd_wait;
      (*any_spi_4bitV2).S4bit.lcd_wait = (*any_spi_4bitV2).S4bit.lcd_elmax;;
    }
  WP_LcdS4V2Strobe(any_spi_4bitV2, qx);
  /* Reset lcd_wait? */
  if (regl == LCD_RETURN_HOME_L) (*any_spi_4bitV2).S4bit.lcd_wait = old_lcd_wait;
}

void WP_LcdS4V2WriteData(lcd_spi_4bitV2 *any_spi_4bitV2,
			 unsigned int data,
			 char qx)
{
  unsigned int datah,datal;
  datah = data & 0xf0;
  datal = data & 0x0f;
  (*any_spi_4bitV2).lcd_S4data = SPI4BIT_LCD_DMASK + datah;
  WP_LcdS4V2Strobe(any_spi_4bitV2, qx);
  (*any_spi_4bitV2).lcd_S4data = SPI4BIT_LCD_DMASK + (datal * 0x10);
  WP_LcdS4V2Strobe(any_spi_4bitV2, qx);
}

void WP_LcdS4V2HomeClear(lcd_spi_4bitV2 *any_spi_4bitV2,
			 char qx)
{
  unsigned int regh,regl;
  regh = LCD_DISPLAY_OFF_H;
  regl = LCD_DISPLAY_OFF_L;
  WP_LcdS4V2WriteRegister(any_spi_4bitV2, regh, regl, qx);
...
void WP_LcdS4V2WriteMessage(lcd_spi_4bitV2 *any_spi_4bitV2,
			    char qx,
			    char *message)
{
  int i,bite;
  ...
  while (*lp1 != NUL)
    {
      ...
      WP_LcdS4V2WriteData(any_spi_4bitV2, data, qx);
      lp1++;
      i++;
      if (i == (*any_spi_4bitV2).S4bit.lcd_nc)
	{
	  regh = LCD_RETURN_HOME_H;
	  regl = LCD_RETURN_HOME_L;
	  WP_LcdS4V2WriteRegister(any_spi_4bitV2, regh, regl, qx);
	  bite = LCD_SET_DDRAM + 
	    ((*any_spi_4bitV2).S4bit.lcd_ddram_idx[1] & LCD_DDRAM_ADDR_MASK);
	  regh = (bite & 0xf0) / 0x10;
	  regl = bite & 0x0f;
	  WP_LcdS4V2WriteRegister(any_spi_4bitV2, regh, regl, qx);
	}
	 ...
	 char **argv)
{
  ...
  int *myspifd;
  int wP_SPIfd[2];
  unsigned int mregh,mregl,spidata,spiregr;
#endif /* HAVE_WIRINGPISPI_H */
 ...
 setup_RCSlist();
  ...
  if (spimode == SYSMODE)
	...
	case WRGRTST:
	  {
	    C2S_gpio_g_mode(current_spi_4bitV2.lcd_e_gpio,
			    GPIO_MODE_IN,
			    xflag);
	    current_spi_4bitV2.lcd_S4data = SPI4BIT_LCD_RMASK; /* Implies BCKLT2 */
	    if (bkltmode == BCKLT0) 
	      current_spi_4bitV2.lcd_S4data =
		current_spi_4bitV2.lcd_S4data | SPI4BIT_LCD_BL_LVL0_OFF;
	    if (bkltmode == BCKLT1)
	      { 
		current_spi_4bitV2.lcd_S4data =
		  current_spi_4bitV2.lcd_S4data | SPI4BIT_LCD_BL_LVL2_OFF;
		current_spi_4bitV2.lcd_S4data =
		  current_spi_4bitV2.lcd_S4data & SPI4BIT_LCD_BL_LVL1_ON;
	      }
	    if (bkltmode == BCKLT3)
	      current_spi_4bitV2.lcd_S4data =
		current_spi_4bitV2.lcd_S4data & SPI4BIT_LCD_BL_LVL3_ON;
	    C2S_sechob2spidev(current_spi_4bitV2.lcd_data.spidev,
			      current_spi_4bitV2.lcd_S4data,
			      xflag);
	    break;
  ...
  if (spimode == WPIMODE)
      ...
      if ((wP_SPIfd[1]=wiringPiSPISetup(1, 500000)) < 0)
	{
      ...
      if (*myspifd > 0)
	{
	  current_spi_4bitV2.lcd_data.spifd = myspifd;
	  printf("*current_spi_4bitV2.lcd_data.spifd = %d\n",
		 *current_spi_4bitV2.lcd_data.spifd);
	}
      switch(demode)
	{
	case CLRHOME:
	...
	case WRGRTST:
	  {
	    current_spi_4bitV2.lcd_S4data = SPI4BIT_LCD_RMASK; /* Includes BCKLT2 mode */ 
	    if (bkltmode == BCKLT0)
	      current_spi_4bitV2.lcd_S4data =
		current_spi_4bitV2.lcd_S4data | SPI4BIT_LCD_BL_LVL0_OFF;
	    if (bkltmode == BCKLT1)
	    ...
	    if (bkltmode == BCKLT3)
	      current_spi_4bitV2.lcd_S4data =
		current_spi_4bitV2.lcd_S4data & SPI4BIT_LCD_BL_LVL3_ON;
	    WP_write_byte(*current_spi_4bitV2.lcd_data.spifd,
			  current_spi_4bitV2.lcd_S4data, 
			  xflag);	  }
	default: break;
	}
  ...
  if (spimode == SYSMODE)
	...
	case WDTATST:
	  {
	    ...
	    for (i=0; i<16; i++)
	      {
		j = i +0x30;
		C2S_LcdS4V2WriteData_Test(j, xflag);
	      }
	    /* Register Select = instruction register */
	    current_spi_4bitV2.lcd_S4data = 
	      current_spi_4bitV2.lcd_S4data & SPI4BIT_LCD_RS_LOW;
	    /* Clear data nibble ... */
	    current_spi_4bitV2.lcd_S4data = current_spi_4bitV2.lcd_S4data & 0x0f;
	    C2S_sechob2spidev(current_spi_4bitV2.lcd_data.spidev,
			      current_spi_4bitV2.lcd_S4data, 
			      xflag);
	    break;
	...
	case WRGRTST:
	  {
	    if (!tflag) xflag=FALSE;
	    C2S_LcdS4V2WriteRegister_Test(LCD_DISPLAY_OFF_H,
					  LCD_DISPLAY_OFF_L,
					  xflag);
	    C2S_LcdS4V2WriteRegister_Test(LCD_DISPLAY_ON_CURSOR_BLINK_H,
					  LCD_DISPLAY_ON_CURSOR_BLINK_L,
					  xflag);
	    C2S_LcdS4V2WriteRegister_Test(LCD_CLEAR_DISPLAY_H,
					  LCD_CLEAR_DISPLAY_L,
					  xflag);	      
  ...
  if (spimode == WPIMODE)
      ...
      if (tflag)
	{
	  ...
	  mehmin = current_spi_4bitV2.S4bit.lcd_ehmin;
	  current_spi_4bitV2.S4bit.lcd_ehmin = 50000;
	  /* and wait for 500000 micro-seconds */
	  mwait = current_spi_4bitV2.S4bit.lcd_wait;
	  current_spi_4bitV2.S4bit.lcd_wait = 500000;
	}
      switch(demode)
	{
	case FSI4TST:
	  {
	    for (i=0; i<4; i++) WP_LcdS4V2F4TStrobe(&current_spi_4bitV2, i, xflag);
	    WP_LcdS4V2WriteRegister(&current_spi_4bitV2, 
				  LCD_DISPLAY_OFF_H,
				  LCD_DISPLAY_OFF_L,
				  xflag);
	    WP_LcdS4V2WriteRegister(&current_spi_4bitV2,
	...
	case STRBTST:
	  {
	    for (i=0; i<5; i++) WP_LcdS4V2Strobe(&current_spi_4bitV2, xflag);
	    break;
	  }
	case WDTATST:
	  {
	    ...
	    for (i=0; i<16; i++)
	      {
		j = i +0x30;
		WP_LcdS4V2WriteData(&current_spi_4bitV2,
				    j,
				    xflag);
	      }
	    */
	    WP_LcdS4V2HomeClear(&current_spi_4bitV2, xflag);
	    sleep(1);
	    for (i=0; i<26; i++)
	      {
		j = i + 65;
		WP_LcdS4V2WriteData(&current_spi_4bitV2,
				  ...
				  xflag);
		sleep(1);
		if (i == current_spi_4bitV2.S4bit.lcd_nc)
		  WP_LcdS4V2WriteRegister(&current_spi_4bitV2,
	    ...
	    WP_LcdS4V2WriteRegister(&current_spi_4bitV2,
				  ...
				  xflag);		
	    bite = LCD_SET_DDRAM + 
	      (current_spi_4bitV2.S4bit.lcd_ddram_idx[1] & LCD_DDRAM_ADDR_MASK);
	...
	case WLINTST:
	  {
	      ...
	      case LCM0802:
		{
		  sprintf(message_buffer, 
			  "0x%02x #1", 
			  current_spi_4bitV2.S4bit.lcd_ddram_idx[0]);
		  WP_LcdS4V2WriteMessage(&current_spi_4bitV2, xflag, message_buffer);
		  bite = LCD_SET_DDRAM + 
		    (current_spi_4bitV2.S4bit.lcd_ddram_idx[1] & LCD_DDRAM_ADDR_MASK);
		  mregh = (bite & 0xf0) / 0x10;
		  mregl = bite & 0x0f;
		  ...
		  sprintf(message_buffer, 
			  "0x%02x #2", 
			  current_spi_4bitV2.S4bit.lcd_ddram_idx[1]);
		  WP_LcdS4V2WriteMessage(&current_spi_4bitV2, xflag, message_buffer);
		  break;
		}
	      case LCM1604:
		{
		  ...
		  WP_LcdS4V2WriteMessage(&current_spi_4bitV2, xflag, message_buffer);
		  bite = LCD_SET_DDRAM + 
		    (current_spi_4bitV2.S4bit.lcd_ddram_idx[2] & LCD_DDRAM_ADDR_MASK);
		  mregh = (bite & 0xf0) / 0x10;
		  mregl = bite & 0x0f;
		  WP_LcdS4V2WriteRegister(&current_spi_4bitV2,
					  mregh,
		  ...
		  sprintf(message_buffer, 
			  "0x%02x LCM1604  #3", 
			  current_spi_4bitV2.S4bit.lcd_ddram_idx[2]);
		  WP_LcdS4V2WriteMessage(&current_spi_4bitV2, xflag, message_buffer);
		  bite = LCD_SET_DDRAM + 
		    (current_spi_4bitV2.S4bit.lcd_ddram_idx[3] & LCD_DDRAM_ADDR_MASK);
		  mregh = (bite & 0xf0) / 0x10;
		  mregl = bite & 0x0f;
		  WP_LcdS4V2WriteRegister(&current_spi_4bitV2,
					  mregh,
		  ...
		  sprintf(message_buffer, 
			  "0x%02x +CSRx16  #4", 
			  current_spi_4bitV2.S4bit.lcd_ddram_idx[3]);
		  WP_LcdS4V2WriteMessage(&current_spi_4bitV2, xflag, message_buffer);
		  break;
		}
	      case LCM2004:
	...
	case WMSGTST:
	  {
	    ...
	    WP_LcdS4V2WriteMessage(&current_spi_4bitV2, xflag, buffer4);
		...
		for (j=0; j<30; j++)
		  {
		    ...
		    get_timenow(timenowis);
		    WP_LcdS4V2WriteMessage(&current_spi_4bitV2, xflag, timenowis);
		    sc = timenowis[7];
      ...
      if (tflag)
	{
	  ...
	  current_spi_4bitV2.S4bit.lcd_ehmin = mehmin;
	  current_spi_4bitV2.S4bit.lcd_wait  = mwait;
	}
    }
#endif /* HAVE_WIRINGPISPI_H */
#endif /* HAVE_WIRINGPI_H */
...
#ifdef HAVE_WIRINGPI_H
      current_spi_4bitV2.lcd_S4data = SPI4BIT_LCD_RMASK; /* Implies BCKLT2 */      
      if (bkltmode == BCKLT0) current_spi_4bitV2.lcd_S4data =
				current_spi_4bitV2.lcd_S4data | SPI4BIT_LCD_BL_LVL0_OFF;
      if (bkltmode == BCKLT1)
	{ 
	  current_spi_4bitV2.lcd_S4data =
      ...
      if (bkltmode == BCKLT3) current_spi_4bitV2.lcd_S4data =
				current_spi_4bitV2.lcd_S4data & SPI4BIT_LCD_BL_LVL3_ON;
      C2S_sechob2spidev(current_spi_4bitV2.lcd_data.spidev,
			current_spi_4bitV2.lcd_S4data, 
			xflag);
 ...
 * Initial revision
 *
 * Revision 1.6  2017/08/14 19:46:32  pi
 * sudo echo --> printf "octal" workaround now in force (see ../common/spi_wraps.c)
 *
 * Revision 1.5  2017/07/01 18:51:16  pi
 * -1604 support etc. added. SCMVersion 0.02
 *
 * Revision 1.4  2017/07/01 18:32:21  pi
 * Interim commit
 *
 * Revision 1.3  2017/07/01 18:00:23  pi
 * Interim commit
 *
 * Revision 1.2  2017/07/01 15:30:31  pi
 * Interim commit
 *
 * Revision 1.1  2015/11/04 22:30:20  pi
 * Initial revision
 *
 * Revision 1.6  2014/11/15 20:32:35  pi
 * SCMVersion 0.01
 *
 * Revision 1.5  2014/11/14 21:15:27  pi
 * Interim commit
 *
 * Revision 1.4  2014/11/14 20:00:23  pi
 * Interim commit
 *
 * Revision 1.3  2014/11/14 19:30:39  pi
 * Interim commit
 *
 * Revision 1.2  2014/11/14 18:53:55  pi
 * Interim commit
 *
 * Revision 1.1  2014/11/14 18:23:30  pi
 * Initial revision
 *
 *
 *
