/* GLCD (128x64 pixels - QC12864B/ST7920), SPI-to-parallel 8-Bit 
 * (74HC595-based) interface, DEMOnstration code
 * $Id: glcd_spi8b_demo.c,v 1.4 2018/03/31 21:32:45 pi Exp $
 */
...
static char RCSFile[]     = "$RCSfile: glcd_spi8b_demo.c,v $";
static char RCSId[]       = "$Id: glcd_spi8b_demo.c,v 1.4 2018/03/31 21:32:45 pi Exp $";
  ...
  do
      ...
      if (l > 1)
	{
	  ...
	  switch(what)
	    ...
	    case BCKLIGHT:
	      {
		if (strcasecmp(buffer2, "on") == 0) blflag = TRUE;
		else blflag = FALSE;
		break;
	      }
...
#ifdef HAVE_WIRINGPI_H
  strcat(buffer1, "         -backlight on|off, \n");
	 ...
	 char **argv)
{
 ...
 setup_RCSlist();
  ...
  if (argc > 1)
      ...
      for (i=1; i<argc; i++)
	{
	  ...
	  switch(what)
	    ...
	    case BCKLIGHT:
	      {
		if (strcasecmp(p2, "on") == 0) blflag = TRUE;
		else blflag = FALSE;
		break;
	      }
  ...
  if (vflag) printf("XBMdump completed ...\n");
  ...
  if (igflag)
    {
      ...
      C2S_gpio_g_mode(current_spi2p_8bit.glcd_e_gpio,
		      GPIO_MODE_OUT,
		      xflag);
      C2S_gpio_g_mode(current_spi2p_8bit.glcd_bl_gpio,
		      GPIO_MODE_OUT,
		      xflag);
      ...
      C2S_gpio_g_write(current_spi2p_8bit.glcd_e_gpio,
		       ...
		       xflag);
      /* BackLight on */
      C2S_gpio_g_write(current_spi2p_8bit.glcd_bl_gpio,
		       GPIO_HIGH,
		       xflag);
  ...
  if (spimode == SYSMODE)
	...
	case WRGRTST:
	  {
	    if ((demode != BLTST) && !tflag) xflag=FALSE;
	    C2S_gpio_g_mode(current_spi2p_8bit.glcd_data.goe_gpio,
			    GPIO_MODE_OUT,
			    xflag);
	    C2S_gpio_g_mode(current_spi2p_8bit.glcd_data.scl_gpio,
	...
	case WMSGTST:
#endif /* HAVE_WIRINGPISPI_H */
	  {
	    ...
	    C2S_gpio_export(current_spi2p_8bit.glcd_e_gpio,
			    GPIO_XOUT,
			    xflag);
	    C2S_gpio_export(current_spi2p_8bit.glcd_bl_gpio,
			    GPIO_XOUT,
			    xflag);
  ...
  if (blflag) C2S_gpio_g_write(current_spi2p_8bit.glcd_bl_gpio,
			       GPIO_HIGH,
			       xflag);
#endif /* HAVE_WIRINGPI_H */
		...
		for (j=0; j<400; j++)
		  {
		    ...
		    l = 10 - i;
		    /* BackLight off */
		    WP_digitalWrite(current_spi2p_8bit.glcd_bl_gpio,
				    GPIO_LOW,
				    xflag);
		    delay(k);
		    /* BackLight on */
	...
	case WMSGTST:
	  {
	    ...
	    strcpy(buffer4, "QC12864B, HC0816Character set.  Running on:     ");
 ...
 * Initial revision
 *
 * Revision 1.3  2017/09/10 22:41:50  pi
 * Extra Verbose diagnostics added
 *
 * Revision 1.2  2017/08/14 20:00:43  pi
 * sudo echo --> printf "octal" workaround now in force (see ../common/spi_wraps.c)
 *
 * Revision 1.1  2015/11/04 22:30:20  pi
 * Initial revision
 *
 * Revision 1.34  2015/08/15 16:42:24  pi
 * Minor tidy up
 *
 * Revision 1.33  2014/10/11 21:52:29  pi
 * Revised strobe timings & mechanism
 *
 * Revision 1.32  2014/10/07 18:25:14  pi
 * -IRST option now conditionally compiled (disabled by default)
 *
 * Revision 1.31  2014/10/06 19:20:11  pi
 * Interim commit
 *
 * Revision 1.30  2014/10/03 23:01:24  pi
 * Some XPM support for UDGC's
 *
 * Revision 1.29  2014/10/01 20:27:19  pi
 * Initial CGRAM character handling
 *
 * Revision 1.28  2014/09/30 13:18:34  pi
 * Interim commit
 *
 * Revision 1.27  2014/09/28 16:32:52  pi
 * -XDMP --> +XBM and +XPM option added
 *
 * Revision 1.26  2014/09/23 20:57:27  pi
 * Interim commit
 *
 * Revision 1.25  2014/09/21 21:30:39  pi
 * Interim commit
 *
 * Revision 1.24  2014/09/20 20:03:42  pi
 * Interim commit
 *
 * Revision 1.23  2014/09/15 22:33:36  pi
 * Interim commit
 *
 * Revision 1.22  2014/09/15 22:15:54  pi
 * Interim commit
 *
 * Revision 1.21  2014/09/15 18:44:18  pi
 * Interim commit
 *
 * Revision 1.20  2014/09/14 21:02:44  pi
 * Interim commit
 *
 * Revision 1.19  2014/09/10 22:40:26  pi
 * Interim commit
 *
 * Revision 1.18  2014/09/09 20:43:29  pi
 * Interim commit
 *
 * Revision 1.17  2014/09/09 18:02:21  pi
 * Interim commit
 *
 * Revision 1.16  2014/09/08 21:51:17  pi
 * Interim commit
 *
 * Revision 1.15  2014/09/07 20:48:49  pi
 * Interim commit
 *
 * Revision 1.14  2014/09/06 21:56:02  pi
 * Strobe delay bug-fix. -GDST option working.
 * SCMVersion 0.01
 *
 * Revision 1.13  2014/09/05 17:06:36  pi
 * Interim commit
 *
 * Revision 1.12  2014/09/03 21:41:21  pi
 * Interim commit. -F8XT option preliminary tests
 * (STANDBY and REVERSE instructions)
 *
 * Revision 1.11  2014/09/03 20:28:57  pi
 * Interim commit. Improved -WMT option.
 *
 * Revision 1.10  2014/08/29 23:18:35  pi
 * Improved -WDT option (all chars. displayed in two "pages")
 *
 * Revision 1.9  2014/08/28 20:03:14  pi
 * Interim commit. Strobe/WriteRegister timing logic/mechanism improved/fixed
 *
 * Revision 1.8  2014/08/27 22:21:35  pi
 * Interim commit
 *
 * Revision 1.7  2014/08/27 18:44:50  pi
 * Interim commit. -CH and font options added. Strobe delay bug fixes.
 *
 * Revision 1.6  2014/08/21 22:23:50  pi
 * Some basic (-WDT) character writing tests now working etc.
 *
 * Revision 1.5  2014/08/21 17:59:25  pi
 * Interim commit
 *
 * Revision 1.4  2014/08/21 15:38:59  pi
 * Interim commit
 *
 * Revision 1.3  2014/08/20 19:59:44  pi
 * Interim commit
 *
 * Revision 1.2  2014/08/19 22:23:49  pi
 * Interim commit
 *
 * Revision 1.1  2014/08/19 18:54:13  pi
 * Initial revision
 *
 *
 */
