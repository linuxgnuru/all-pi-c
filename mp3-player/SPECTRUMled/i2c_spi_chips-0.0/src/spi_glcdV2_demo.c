/* SPI Graphics LCD (nokia5110-like), Version 2 pinout, DEMOnstration code
 * $Id: spi_glcdV2_demo.c,v 1.2 2018/03/31 21:32:45 pi Exp $
 */
...
static char RCSFile[]     = "$RCSfile: spi_glcdV2_demo.c,v $";
static char RCSId[]       = "$Id: spi_glcdV2_demo.c,v 1.2 2018/03/31 21:32:45 pi Exp $";
...
/* Set up USAGE message */
{
  ...
  strcat(buffer1, "         -IG  {Initialise gpio's, check backlight & Gpio readall},\n");
#ifdef HAVE_WIRINGPISPI_H
	 ...
	 char **argv)
{
...
#endif /* HAVE_WIRINGPISPI_H */
  init_colpix_store(&nokia5110v2_buffer);
  ...
  if (xpmfopen)
	...
	npix = Read_Xpm_Colpix8_Data(&nokia5110v2_buffer, vflag);
  ...
  if (igflag)
    {
      init_pcd8544(&nokia5110v2,
		   25,
		   24,
		   4,
		   spidev, 
		   "Nokia5110V2");
      echo_pcd8544_status(nokia5110v2);
      echo_colpix_store_status(&nokia5110v2_buffer);
#ifdef HAVE_WIRINGPI_H
      C2S_gpio_g_mode(nokia5110v2.rst_gpio,
		      GPIO_MODE_OUT,
		      xflag);
      /* ReSeT inactive */
      C2S_gpio_g_write(nokia5110v2.rst_gpio,
		       GPIO_HIGH,
		       xflag);
      C2S_gpio_g_mode(nokia5110v2.dc_gpio,
		      GPIO_MODE_OUT,
		      xflag);
      /* Data/¬Command = Command */
      C2S_gpio_g_write(nokia5110v2.dc_gpio,
		       GPIO_LOW,
		       xflag);
      if (nokia5110v2.bl_gpio != GPIO_UNASSIGNED)
	{
	  C2S_gpio_g_mode(nokia5110v2.bl_gpio,
			  GPIO_MODE_OUT,
			  xflag);
	  /* BackLight on */
	  C2S_gpio_g_write(nokia5110v2.bl_gpio,
			   PCD8544_BL_CA_PNP_ON,
			   xflag);
	  sleep(5);
	  /* BackLight off */
	  C2S_gpio_g_write(nokia5110v2.bl_gpio,
			   PCD8544_BL_CA_PNP_OFF,
			   xflag);
	}
  ...
  C2S_gpio_g_mode(nokia5110v2.rst_gpio, GPIO_MODE_OUT, xflag);
  /* O.K. to export Data/¬Command gpio ... */
  C2S_gpio_export(nokia5110v2.dc_gpio, GPIO_XOUT, xflag);
  /* An exported the BackLight gpio is not saved on exit by kernel 3.18... */
  /* ... so use SYSTEM method instead! */
  /* if (nokia5110v2.bl_gpio != GPIO_UNASSIGNED)
     C2S_gpio_export(nokia5110v2.bl_gpio, GPIO_XOUT, xflag); */
  if (nokia5110v2.bl_gpio != GPIO_UNASSIGNED)
    C2S_gpio_g_mode(nokia5110v2.bl_gpio, GPIO_MODE_OUT, xflag);
  if (xflag)
    {
  ...
  /* Exporting the ReSeT gpio leads to spurious reset pulses so ... */
  /* ... use SYSTEM method instead! */
  C2S_gpio_g_write(nokia5110v2.rst_gpio, GPIO_HIGH, xflag);
  /* Data/¬Command = Command mode */
  WP_digitalWrite(nokia5110v2.dc_gpio, GPIO_LOW, xflag);
  /* BackLight on? */
  /* An exported the BackLight gpio is not saved on exit by kernel 3.18... */
  /* ... so use SYSTEM method instead! */
  /* if (blflag && (nokia5110v2.bl_gpio != GPIO_UNASSIGNED))
     WP_digitalWrite(nokia5110v2.bl_gpio, PCD8544_BL_CA_PNP_ON, xflag); */
  if (blflag && (nokia5110v2.bl_gpio != GPIO_UNASSIGNED))
	  C2S_gpio_g_write(nokia5110v2.bl_gpio, PCD8544_BL_CA_PNP_ON, xflag);    
  ...
  if (*myspifd > 0)
    {
      nokia5110v2.spifd = myspifd;
      printf("*nokia5110v2.spifd = %d\n",
	     *nokia5110v2.spifd);
    }
#endif /* HAVE_WIRINGPISPI_H */
#endif /* HAVE_WIRINGPI_H */
  ...
  switch(demode)
    ...
    case CLRHOME:
      {
	WP_FS_ClearHome(&nokia5110v2, xflag);
	break;
      }
    case INVERSE:
    case NORMAL:
      {
	nokia5110v2.dcval = PCD8544_FUNCTIONSET;
	WP_FS_WriteCommandByte(&nokia5110v2, xflag);
	nokia5110v2.dcval = PCD8544_DISPLAYCONTROL | dispmode;
	WP_FS_WriteCommandByte(&nokia5110v2, xflag);
	break;
      }
    case RESETINIT:
	...
	WP_C2S_GlcdReset_Test(nokia5110v2.rst_gpio, xflag);
	nokia5110v2.dcval = PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION;
	WP_FS_WriteCommandByte(&nokia5110v2, xflag);
	nokia5110v2.dcval = PCD8544_SETBIAS | 0x04;
	WP_FS_WriteCommandByte(&nokia5110v2, xflag);
	nokia5110v2.dcval = PCD8544_SETVOP | convop;
	WP_FS_WriteCommandByte(&nokia5110v2, xflag);
	nokia5110v2.dcval = PCD8544_FUNCTIONSET;
	WP_FS_WriteCommandByte(&nokia5110v2, xflag);
	nokia5110v2.dcval = PCD8544_DISPLAYCONTROL | PCD8544_DISPLAYNORMAL;
	WP_FS_WriteCommandByte(&nokia5110v2, xflag);
	break;
      }
    case SETCON0:
	...
	WP_FS_ClearHome(&nokia5110v2, xflag);
	for (i=0; i<64; i++)
	  WP_FS_WriteFontDataBytes(&nokia5110v2, i, font8x5, xflag);
	sleep(10);
	WP_FS_ClearHome(&nokia5110v2, xflag);
	for (i=64; i<128; i++)
	  WP_FS_WriteFontDataBytes(&nokia5110v2, i, font8x5, xflag);
    ...
    case WGRTST:
      {
	WP_FS_ClearHome(&nokia5110v2, xflag);
	WP_FS_WriteGraphicsDataBytes(&nokia5110v2, &nokia5110v2_buffer, xflag);
	break;
      }
    case WMSGTST:
      {
	WP_FS_ClearHome(&nokia5110v2, xflag);
	sleep(1);
	strcpy(buffer4, nokia5110v2.name);
	strcat(buffer4, "   ");
	l = strlen(buffer4);
	...
	printf("%s\n", buffer4);
	WP_FS_WriteMessage(&nokia5110v2,
			   font8x5,
			   xflag,
			   buffer4);
	break;
  ...
  /* ... so use SYSTEM method instead! */
  /* if (!blflag && (nokia5110v2.bl_gpio != GPIO_UNASSIGNED))
     WP_digitalWrite(nokia5110v2.bl_gpio, PCD8544_BL_CA_PNP_OFF, xflag); */
  if (!blflag && (nokia5110v2.bl_gpio != GPIO_UNASSIGNED))
    C2S_gpio_g_write(nokia5110v2.bl_gpio, PCD8544_BL_CA_PNP_OFF, xflag);    
#ifdef HAVE_WIRINGPISPI_H
  /* Data/¬Command = Command mode */
  WP_digitalWrite(nokia5110v2.dc_gpio, GPIO_LOW, xflag);
  if (eflag || vflag) 
 ...
 * Initial revision
 *
 * Revision 1.6  2015/02/10 13:52:45  pi
 * BackLight gpio no longer exported (kernel 3.18... fix)
 * SCMVersion 0.02
 *
 * Revision 1.5  2015/01/25 13:03:04  pi
 * Function moved into spi_PCD8544
 *
 * Revision 1.4  2015/01/22 18:12:02  pi
 * -WMT option added
 *
 * Revision 1.3  2015/01/16 23:29:02  pi
 * SCMVersion 0.01
 *
 * Revision 1.2  2015/01/15 16:40:41  pi
 * Basic functions implemented.
 * SCMVersion 0.0
 *
 * Revision 1.1  2015/01/15 14:56:28  pi
 * Initial revision
 *
 *
 *
