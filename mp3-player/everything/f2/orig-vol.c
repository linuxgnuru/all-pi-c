...
double get_normalized_volume(snd_mixer_elem_t *elem) {
    ...
    if (err < 0) { printf("Error getting volume\n"); return 0; }
...
void set_normalized_volume(snd_mixer_elem_t *elem, double volume) {
    ...
    err = snd_mixer_selem_get_playback_dB_range(elem, &min, &max);
    if (err < 0) { printf("Error setting volume\n"); return; }
    // Perceived 'loudness' does not scale linearly with the actual decible level
    // it scales logarithmically
...
int main(int argc, char **argv) {
    ...
    if (wiringPiSetup() == -1) { fprintf(stdout, "wiringPiSetup failure: %s\n", strerror(errno)); return EXIT_FAILURE; }
    ...
    if (snd_mixer_open(&handle, 0) < 0) { printf("Error openning mixer\n"); exit(1); }
    if (snd_mixer_attach(handle, card) < 0) { printf("Error attaching mixer\n"); snd_mixer_close(handle); exit(1); }
    if (snd_mixer_selem_register(handle, NULL, NULL) < 0) { printf("Error registering mixer\n"); snd_mixer_close(handle); exit(1); }
    if (snd_mixer_load(handle) < 0) { printf("Error loading mixer\n"); snd_mixer_close(handle); exit(1); }
    elem = snd_mixer_find_selem(handle, sid);
    if (!elem) { printf("Error finding simple control\n"); snd_mixer_close(handle); exit(1); }
