...
{
	int oldvalue;
	oldvalue = abs(analogRead(MY_8591) - 255);
	return oldvalue;
}
...
static struct mpd_connection *setup_connection()
{
	...
	conn = mpd_connection_new("10.0.5.118", 6600, 0);
	if (conn == NULL) 
	{
		fputs("Out of memory\n", stderr);
		exit(EXIT_FAILURE);
...
int main(int argc, char **argv)
{
	...
	*/
	int oldvalue = analog_volume();
	/*
	truct encoder *song_selector = setupencoder(13,12);
	if (song_selector == NULL) { exit(1); }
	int oldsongvalue = song_selector->value;
	*/
	struct lcd lcd = lcd_init(3, 14, 4, 12, 13, 6);
	struct button *button = setup_button(15, play_pause_callback);
	if (button == NULL) { exit(1); }
	setup_button(16, mute_unmute_callback);
	setup_button(10, next_song_callback);
	setup_button(11, prev_song_callback);
	struct led play_indicator = led_init(7, -1, -1);
	struct led mute_indicator = led_init(5, -1, -1);
	...
	while (1)
	{
		if (oldvalue != analog_volume())
		{
			int change = analog_volume() - oldvalue;
			int chn = 0;
