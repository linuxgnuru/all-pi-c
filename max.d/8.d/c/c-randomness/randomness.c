...
const char numOfDevices = 8;

unsigned char ledData[8][8] = {
...
{
    if (addr < 0 || addr > 8 || col < 0 || col > 7 || row < 0 || row > 7) return;
...
_Bool bitRead(int addr, int col, int row)
{
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        //{
            //lm = cm;
            for (int addr = 7; addr > -1; addr--)
            {
                for (col = 0; col < 7; col++)
                {
                    int ran = rand() % 9;
                    doGraph(addr, ran, col);
                }
            }
            delay(5);
        //}
