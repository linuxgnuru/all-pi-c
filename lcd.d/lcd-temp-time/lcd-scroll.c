...
void scrollMessage (int line, int width)
{
  ...
  static int timer = 0 ;
  int err_no;
  char my_message_head[13] = "Temperature ";
  char temp_c[7];
  char my_message[20];
  ...
  timer = millis () + 200 ;
  err_no = Temp(temp_c);
  strcpy(my_message, my_message_head);
  strcat(my_message, temp_c);

  strncpy (buf, &my_message [position], width) ;
  buf [width] = 0 ;
  lcdPosition (lcdHandle, 0, line) ;
  lcdPuts     (lcdHandle, buf) ;

  if (++position == (strlen (my_message) - width))
    position = 0 ;
}
...
int main (int argc, char *argv[])
{
  ...
  //  lcdHandle = lcdInit (rows, cols,  4, 11, 10, 4,  5,  6, 7,0,0,0,0) ;
      lcdHandle = lcdInit (rows, cols,  4,  3, 14, 4, 12, 13, 6,0,0,0,0) ;
  }
  else
  ...
  lcdPosition (lcdHandle, 0, 0) ; lcdPuts (lcdHandle, "John Wiggins") ;
  lcdPosition (lcdHandle, 0, 1) ; lcdPuts (lcdHandle, "  jcwiggi@gmail.com  ") ;
